...
export async function firebaseBootstrap(): Promise<void> {
  ...
  try {
    await firebase.auth().signInWithEmailAndPassword(process.env.APP_FIREBASE_LOGIN, process.env.APP_FIREBASE_PASSWORD);
  } catch (error) {
    const errorCode = error.code;
    const errorMessage = error.message;

    // tslint:disable-next-line:no-console
    console.log(`Error has occured. Error code: ${errorCode}, message: ${errorMessage}.`);
  }
}
